      ...
      <Box>
        <Stack spacing={1} sx={{ mb: 2 }}>
          <TextField
            ...
            helperText={
              name.length > 0 && (name.length < 3 || name.length > 140)
                ? "Debe tener entre 3 y 140 caracteres"
                : ""
            }
            value={name}
          ...
          >
            ...
            >
              {enumToOptions(AreaName, AreaNameLabel).map(
                ({ value, label }) => (
          ...
          {rows.map((row, index) => (
            ...
            >
              <Typography variant="caption" sx={{ fontWeight: 600 }}>
                Fila {index + 1} | {row.columns.length}{" "}
                {row.columns.length > 1 ? "columnas" : "columna"}
              </Typography>
              <Stack
                direction="row"
              ...
              >
                ...
                <IconButton
                  ...
                  disabled={
                    row.columns.length >= (layoutMode === "PORTRAIT" ? 4 : 8)
                  }
